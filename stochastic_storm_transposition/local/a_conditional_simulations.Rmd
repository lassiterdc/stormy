...
f_out_sim_wlevel_with_emp_cdf_subset <- paste0(dir_sst_outs, "r_a_sim_wlevel_cdf_with_multivariate_empcdf_subset.csv")
...
dt_sim_subset = data.table(tibble(Sim) %>% select(max_surge_ft, depth_mm, max_mm_per_hour))

emp_cdf_sim <- empirical_cdf(dt_sim, dt_sim[1,])

dt_results <- data.table()
dt_subset_results <- data.table()
...
for (i in 1:nrow(dt_sim)) {
  ...
  row_values <- dt_sim[i, ]
  row_values_subset <- dt_sim_subset[i,]
  
  emp_cdf_sim <- empirical_cdf(dt_sim, row_values)
  emp_cdf_sim_subset <- empirical_cdf(dt_sim_subset, row_values_subset)
  
  dt_results <- rbind(dt_results, emp_cdf_sim)
  dt_subset_results <- rbind(dt_subset_results, emp_cdf_sim_subset)
...
tibble(dt_subset_results) %>% write_csv(f_out_sim_wlevel_with_emp_cdf_subset)

# also simulate on just three of the variables
```
